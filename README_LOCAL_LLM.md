...
3. **モデルのロードと設定**
   - ダウンロードしたモデルを選択
   - **Context Length**: `40000`（推奨）または `8192`以上
     - **重要**: 4096トークンでは不足します。エラーメッセージで「4375 tokens when context the overflows」と表示されるため、少なくとも8192に設定してください
     - 各casestudyファイルは平均約1200トークン、プロンプト全体（システム指示、分類オプション、各クラスの説明を含む）で4000-5000トークン以上になる可能性があります
     - 安全のため、**8192以上**（推奨: **40000**）に設定してください
   - **GPU Offload**: 
     - **GPU推論（推奨）**: `Max`（全レイヤー、48/48）に設定
     - **CPU推論**: `0`（スライダーを左端に移動）に設定
       - CPU推論の場合は遅くなりますが、GPUメモリを使用しません
       - CPU Thread Pool Sizeを増やす（例: 8-12）ことで若干高速化できます
   - **KVキャッシュをGPUメモリにオフロード**: 
     - GPU推論の場合: `ON`（推奨）
     - CPU推論の場合: `OFF`（GPUメモリを使用しないため）
   - **モデルをメモリに保持**: `ON`（推奨、再読み込み時間の短縮）
   - **Flash Attention**: 
     - GPU推論の場合: `ON`（推奨、パフォーマンス向上）
     - CPU推論の場合: `OFF`（CPUでは効果が限定的）
   - **エキスパート数**: `8`（Qwen3-VL-30Bのデフォルト、MoEモデルのため）
   - **CPU Thread Pool Size**: 
     - GPU推論の場合: `6`（デフォルトで問題なし）
     - CPU推論の場合: `8-12`（推奨、CPUコア数に応じて調整）
   - 「Load」をクリックしてモデルをロード
