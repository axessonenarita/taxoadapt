...
dimension_definitions = {
    ...
    '導入効果': """導入効果: システム導入後に実現した効果・成果。導入前と比較して改善された結果を記述する。
- 定量的効果: 作業時間の削減（例：「50%削減」「年間1,000時間削減」）、コスト削減（例：「年間100万円削減」）、処理件数の増加など
- 定性的効果: 業務効率化、顧客満足度向上、品質向上、ガバナンス強化、意思決定の迅速化、データ活用促進など
- キーワード: 「削減された」「向上した」「実現した」「達成した」「改善された」など、過去形や完了形で表現される効果
- 注意: 導入前の問題ではなく、導入後の結果として得られた効果を記述する。事例の内容から判断する。""",
    ...
    '業務課題': """業務課題: システム導入前に存在していた問題・課題。システム導入により解決すべき問題を記述する。
- 具体的な課題: 属人化、業務効率の低下、承認プロセスの複雑化、情報共有の不足、レガシーシステムの老朽化、コスト増加、品質管理の困難、データ分散、システム統合の必要性など
- キーワード: 「課題」「問題」「困難」「不足」「複雑化」「低下」「増加」など、問題や課題を示す表現
- 注意: 導入後の効果ではなく、導入前に存在していた問題や課題を記述する。事例の内容から判断する。""",
...
3. 導入効果: システム導入後に実現した効果・成果。導入前と比較して改善された結果を記述する。
   - 定量的効果: 作業時間の削減、コスト削減、処理件数の増加など
   - 定性的効果: 業務効率化、顧客満足度向上、品質向上、ガバナンス強化など
   - キーワード: 「削減された」「向上した」「実現した」など、過去形や完了形で表現される効果
   - 注意: 導入前の問題ではなく、導入後の結果として得られた効果を判断する。該当する場合はTrueを返す。
...
6. 業務課題: システム導入前に存在していた問題・課題。システム導入により解決すべき問題を記述する。
   - 具体的な課題: 属人化、業務効率の低下、承認プロセスの複雑化、情報共有の不足、レガシーシステムの老朽化など
   - キーワード: 「課題」「問題」「困難」「不足」など、問題や課題を示す表現
   - 注意: 導入後の効果ではなく、導入前に存在していた問題や課題を判断する。該当する場合はTrueを返す。
...
######################## WIDTH EXPANSION JUDGMENT ########################

width_expansion_judgment_system_instruction = """あなたはタクソノミーの幅方向展開が必要かどうかを判断するアシスタントです。既存の子ノードに分類された論文を見て、新しいカテゴリが必要かどうかを判断してください。

幅方向展開は、既存の子ノードではカバーしきれない新しい概念やトピックが存在する場合に実行すべきです。ただし、無理に新しいカテゴリを作る必要はありません。既存のカテゴリで十分にカバーできている場合は、幅方向展開は不要です。"""

class WidthExpansionJudgmentSchema(BaseModel):
    should_expand: bool
    reasoning: Annotated[str, StringConstraints(strip_whitespace=True, max_length=500)]
    sample_paper_ids: conlist(int, min_length=0, max_length=10)  # 新しいカテゴリが必要そうな論文のID（サンプル）

def width_expansion_judgment_prompt(node, sample_papers, existing_children, nl='\n'):
    """既存の子ノードに分類された論文を見て、新しいカテゴリが必要かどうかを判断するプロンプト"""
    sample_papers_text = []
    for paper_id, paper in sample_papers.items():
        sample_papers_text.append(f"論文ID {paper_id}: タイトル「{paper.title[:100]}...」")
...
<existing_children>
既存の子ノード:
{nl.join([f"- {c_label}: {c.description} (論文数: {len(c.papers)}件)" for c_label, c in existing_children.items()])}
</existing_children>

<sample_classified_papers>
以下の論文は既存の子ノードに分類されています。これらの論文を見て、新しいカテゴリが必要かどうかを判断してください:
{nl.join(sample_papers_text)}
</sample_classified_papers>

</input>

既存の子ノードに分類された論文を見て、以下の点を判断してください：

1. 既存の子ノードではカバーしきれない、新しい概念やトピックが存在するか？
2. その新しい概念やトピックが、十分な数の論文で表現されているか？
3. 無理に新しいカテゴリを作る必要はない。既存のカテゴリで十分にカバーできている場合は、幅方向展開は不要です。

出力は以下のJSON形式で行ってください:
{{
    "should_expand": <新しいカテゴリが必要な場合はTrue、不要な場合はFalse>,
    "reasoning": <判断理由を1-2文で説明>,
    "sample_paper_ids": <新しいカテゴリが必要そうな論文のIDのリスト（最大10件）。should_expandがFalseの場合は空リスト>
}}
